...
> 🟡 **虚拟线程不是语法糖，而是 JVM 层面的真正调度革新**。理解其原理需要搞清楚三个核心概念：

**1. Continuation（续体）**——虚拟线程的"暂停/恢复"基础：

```java
// 伪代码展示 Continuation 的核心思想
// Continuation = 可以被暂停和恢复执行的代码块
Continuation cont = new Continuation(scope, () -> {
    System.out.println("step 1");
    Continuation.yield(scope);  // ★ 暂停：保存整个调用栈到堆内存
    System.out.println("step 2");  // 恢复后从这里继续
});

cont.run();   // 输出 "step 1"，然后 yield 暂停
cont.run();   // 恢复，输出 "step 2"
```

> 🔴 **核心原理**：`yield` 时把当前虚拟线程的整个栈帧（局部变量、操作数栈、返回地址）从平台线程的栈**拷贝到堆内存**（Java 对象）。恢复时再从堆拷贝回栈。这就是为什么虚拟线程的栈可以很小（~1KB起步，动态增长）——它存在堆上，不需要一次性分配 1MB 连续栈空间。

**2. Carrier Thread（载体线程）**——真正执行代码的 OS 线程：

```mermaid
flowchart TB
    subgraph FJP["ForkJoinPool (默认=CPU核数个载体线程)"]
        CT1["Carrier Thread 1<br/>(真正的OS线程)"]
        CT2["Carrier Thread 2"]
        CT3["Carrier Thread 3"]
    end

    VT1["Virtual Thread A<br/>执行 IO..."] -.mount.-> CT1
    VT2["Virtual Thread B<br/>执行计算"] -.mount.-> CT2
    VT3["Virtual Thread C<br/>等待IO完成(已yield)"] -.unmounted<br/>栈在堆中等待.-> HEAP["堆内存"]

    CT1 -->|"A遇到IO阻塞<br/>→ yield → unmount"| CT1_FREE["CT1空闲<br/>可以mount其他VT"]
    CT1_FREE -.->|"mount VT-D"| VTD["Virtual Thread D"]

    style HEAP fill:#feca57
    style CT1_FREE fill:#48dbfb
```

> **文字版总结**：虚拟线程不直接对应 OS 线程。多个虚拟线程轮流 mount 到少量载体线程上执行。遇到阻塞 IO 时，虚拟线程 unmount（让出载体线程），栈保存到堆；IO 完成后重新 mount 到任意空闲载体线程继续执行。这就是 M:N 调度——M 个虚拟线程共享 N 个 OS 线程。

**3. Pin（钉住）问题**——虚拟线程的阿喀琉斯之踵：

```java
// ❌ 在 synchronized 块内做阻塞 IO → 虚拟线程被 pin 住
synchronized (lock) {
    socket.read();  // ★ 这里本应 unmount 让出载体线程
                    // 但因为在 synchronized 内，JVM 无法安全地保存 monitor 状态
                    // 导致虚拟线程无法 unmount → 载体线程也被阻塞 → 退化为 1:1
}

// ✅ 改用 ReentrantLock → 不会 pin
lock.lock();
try {
    socket.read();  // ReentrantLock 状态在堆上，yield 时可以安全保存
} finally { lock.unlock(); }
```

> 🟢 **为什么 synchronized 会 pin？** 因为 `synchronized` 的锁状态记录在对象头 MarkWord 和 OS 级别的 monitor 中，与特定的 OS 线程绑定。如果虚拟线程 unmount（换载体线程），monitor 的 owner 线程就变了——这违反了 monitor 的语义。而 `ReentrantLock` 的 state 是 Java 堆上的变量，随虚拟线程的 Continuation 一起保存/恢复，不依赖 OS 线程身份。
...
#### 8 大操作的交互时序（以 `x = x + 1` 为例）

```mermaid
sequenceDiagram
    participant E as 执行引擎
    participant WM as 工作内存(CPU缓存)
    participant MM as 主内存(堆)

    Note over MM: x = 0 (初始值)
    MM->>WM: ① read: 从主内存读出 x=0
    WM->>WM: ② load: 将 x=0 放入工作内存副本
    WM->>E: ③ use: 将 x=0 传给执行引擎
    E->>E: 执行 x+1 = 1
    E->>WM: ④ assign: 将计算结果 1 写回工作内存
    WM->>WM: ⑤ store: 将 x=1 放入传输通道
    WM->>MM: ⑥ write: 将 x=1 写入主内存
    Note over MM: x = 1 (更新完成)
```

> 🟠 **关键约束规则（不遵守就会出 bug）**：
> - `read` 和 `load` 必须成对出现（不能只 read 不 load）
> - `store` 和 `write` 必须成对出现（不能只 store 不 write）
> - `assign` 后必须最终 `store+write`（不能修改了不刷回主内存——但"最终"的时间不确定，这就是可见性问题的根源！）
> - `lock` 操作会清空工作内存中该变量的副本，重新 `read+load`（这就是 synchronized 保证可见性的原因）
> - `unlock` 之前必须把修改 `store+write` 回主内存（这就是"释放锁时刷新"的底层语义）
>
> **如果没有 volatile/synchronized 强制约束**：JVM 可以延迟 store+write 的时机（写缓冲区积压），导致其他线程长时间看到旧值。
...
#### MESI 状态转换时序（以 volatile 写+读为例）

```mermaid
sequenceDiagram
    participant CPU1 as CPU1 (写线程)
    participant Bus as 总线/互联
    participant CPU2 as CPU2 (读线程)
    participant MM as 主内存

    Note over CPU1: 缓存行状态: S (共享)
    Note over CPU2: 缓存行状态: S (共享)

    CPU1->>CPU1: volatile write: x = 42
    CPU1->>CPU1: 缓存行状态 S→M (已修改)
    CPU1->>Bus: 发送 Invalidate 消息
    Bus->>CPU2: Invalidate 到达
    CPU2->>CPU2: 缓存行状态 S→I (无效)
    CPU2->>Bus: 回复 Invalidate ACK
    Note over CPU1: 写缓冲区刷出(lock前缀强制)

    CPU2->>CPU2: volatile read: 读 x
    CPU2->>CPU2: 发现缓存行是 I (无效)
    CPU2->>Bus: 发送 Read 请求
    Bus->>CPU1: Read 请求到达(CPU1有最新值)
    CPU1->>Bus: 回复数据 x=42
    CPU1->>CPU1: 缓存行状态 M→S
    Bus->>CPU2: 数据 x=42
    CPU2->>CPU2: 加载到缓存，状态=S
    Note over CPU2: 读到最新值 42 ✅
```

> **文字版总结（快速记忆）**：volatile 写 → 锁缓存行+刷写缓冲区+发 Invalidate → 其他 CPU 缓存行失效 → 下次读时 Cache Miss → 从最新位置重新加载。整个机制保证了"写后可见"。
>
> 🟢 **为什么说 volatile 有性能开销？** 每次 volatile 写都触发 Invalidate（跨 CPU 通信，~10ns），每次 volatile 读都可能 Cache Miss（~100ns vs 命中 ~1ns）。所以不要随便给变量加 volatile——只在确实需要跨线程可见性时才用。
...
> 🟢 **为什么读到 0？**（三种可能，任一即可导致）
> 1. **编译器重排**：JIT 编译器看到操作1和操作2没有数据依赖（单线程视角），可能重排为 2→1。reader 看到 flag=true 时，a=1 可能还没执行。
> 2. **CPU 重排（Store-Store）**：即使编译器没重排，CPU 乱序执行可能先执行 flag 的 store（因为 flag 是 boolean 占 1 字节，写更快），再执行 a 的 store。
> 3. **内存可见性延迟**：即使 CPU 按顺序执行了，a=1 可能还在 writer 的写缓冲区中（Store Buffer），尚未刷到主内存/其他 CPU 可见。flag=true 先被 reader 看到（可能因为在不同的缓存行，Invalidate 消息到达顺序不同）。

**修复方式对比**：

| 修复方案 | 代码 | 原理 | 开销 |
|---------|------|------|------|
| volatile flag | `volatile boolean flag` | 操作1 hb 操作2(程序序) hb 操作3(volatile规则) hb 操作4(程序序) → 传递性 | 低(仅flag读写有屏障) |
| synchronized | writer/reader 都加 `synchronized(this)` | 监视器锁规则：unlock hb lock | 中(互斥) |
| 两个都 volatile | `volatile int a; volatile boolean flag` | 每个变量独立保证 | 高(两个变量都有屏障) |

> 🟠 **最优方案**：只给 flag 加 volatile。利用传递性，flag 的 volatile 写会"带上"它之前所有的普通写（操作1）的可见性。这就是上面 7.3 中"利用 volatile 保护非 volatile 变量"的技巧——只需付一个 volatile 的代价，就能保护一批变量的可见性。
...
### 9.5.1 🔴 轻量级锁的 CAS 流程（完整步骤）

> 🔴 **轻量级锁是 synchronized 在低竞争场景下的优化**，理解其流程需要知道"Lock Record"的概念：

**加锁流程（step by step）**：

```mermaid
flowchart TD
    A["线程进入 synchronized 块"] --> B["在当前栈帧中创建 Lock Record<br/>(包含: Displaced Mark Word 槽 + owner 指针)"]
    B --> C["将对象当前的 MarkWord<br/>复制到 Lock Record 的<br/>Displaced Mark Word 中<br/>(保存原始状态用于恢复)"]
    C --> D["CAS 尝试将对象的 MarkWord<br/>替换为指向 Lock Record 的指针<br/>(同时标志位改为 00)"]
    D -->|"CAS 成功"| E["获得轻量级锁 ✅<br/>对象头现在指向自己的栈帧"]
    D -->|"CAS 失败"| F{"MarkWord 中的指针<br/>指向自己的栈帧?"}
    F -->|"是"| G["说明是锁重入<br/>添加一个新的 Lock Record<br/>(Displaced Mark Word=null 标记重入)"]
    F -->|"否"| H["说明有其他线程竞争<br/>自适应自旋等待"]
    H -->|"自旋成功<br/>(对方释放了)"| E
    H -->|"自旋失败<br/>(超过阈值)"| I["膨胀为重量级锁<br/>(创建 ObjectMonitor)"]
...
```mermaid
flowchart TD
    A["线程退出 synchronized 块"] --> B{"Lock Record 的<br/>Displaced Mark Word == null?"}
    B -->|"是"| C["这是重入的 unlock<br/>只是移除一个 Lock Record<br/>(不操作对象头)"]
    B -->|"否"| D["CAS 将 Displaced Mark Word<br/>写回对象的 MarkWord<br/>(恢复原始状态)"]
    D -->|"CAS 成功"| E["释放轻量级锁 ✅"]
    D -->|"CAS 失败"| F["说明锁已膨胀为重量级<br/>(其他线程在等待时触发了膨胀)<br/>走重量级锁的释放逻辑"]

    style E fill:#48dbfb
```

> 🟠 **为什么 CAS 失败说明已膨胀？** 因为如果对象的 MarkWord 仍然指向当前 Lock Record，CAS 一定成功。CAS 失败说明 MarkWord 已经被修改——唯一的可能是其他线程竞争时触发了锁膨胀，MarkWord 被替换为指向 ObjectMonitor 的指针（标志位变成 10）。

**自适应自旋的"自适应"是什么意思？**

> 🟠 **JVM 会根据历史经验动态调整自旋次数**：
> - 如果上次在这个锁上自旋成功了 → 这次允许自旋更多次（乐观预期）
> - 如果上次自旋失败了（最终膨胀）→ 这次减少自旋甚至直接膨胀（悲观预期）
> - 如果锁的持有者线程正在运行（还在临界区内）→ 有理由继续自旋
> - 如果锁的持有者线程被阻塞了 → 立即膨胀（自旋没意义）
>
> 这是 HotSpot JVM 的运行时优化，具体实现在 `adaptiveSpin` 方法中，不是一个固定阈值。
...
#### shouldParkAfterFailedAcquire 的深度理解

> 🟠 **这个方法决定了"获取锁失败后是否应该阻塞"**，是 AQS 中最微妙的逻辑之一：
...
private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
    ...
    if (ws == Node.SIGNAL)
        // ★ 情况1：前驱已经是 SIGNAL → 安全 park
        // 前驱释放锁时一定会 unpark 我
        return true;

    if (ws > 0) {
        // ★ 情况2：前驱被取消了(CANCELLED=1) → 跳过所有取消的节点
        do {
            node.prev = pred = pred.prev;
        } while (pred.waitStatus > 0);
        pred.next = node;
        // 返回 false → 不 park，回到循环重试

    } else {
        // ★ 情况3：前驱状态是 0 或 PROPAGATE → CAS 设为 SIGNAL
        pred.compareAndSetWaitStatus(ws, Node.SIGNAL);
        // 返回 false → 不 park，回到循环再检查一次
        // 为什么不直接 park？因为要给前驱一个"再试一次"的机会
        // （设 SIGNAL 和 释放锁可能并发，如果此时前驱刚好释放了呢？）
    }
    return false;
}
```

> 🔴 **为什么要这么复杂？** 核心目的是 ==确保 park 之前一定有人会 unpark 我==。如果直接 park 而前驱状态不是 SIGNAL，前驱释放锁时不会执行 unparkSuccessor → 当前线程永远被阻塞 → **死锁**！所以必须先把前驱设为 SIGNAL，确认后再 park。这是一个"先布好唤醒机制再睡觉"的防御性编程。

#### addWaiter 的 CAS 入队细节
...
private Node addWaiter(Node mode) {
    ...
    for (;;) {
        Node oldTail = tail;
        if (oldTail != null) {
            // ★ 先设 prev（从后往前遍历能看到的保证）
            node.setPrevRelaxed(oldTail);
            // ★ CAS 设置 tail
            if (compareAndSetTail(oldTail, node)) {
                oldTail.next = node;  // 后设 next（从前往后可能暂时看不到）
                return node;
            }
        } else {
            initializeSyncQueue();  // 懒初始化：创建哨兵 head 节点
        }
    }
}
```

> 🟠 **这解释了为什么 unparkSuccessor 要从尾部向前遍历**：`node.prev` 的设置先于 CAS tail，所以从后往前遍历（通过 prev）一定能看到所有节点。而 `oldTail.next = node` 在 CAS 之后设置，从前往后遍历可能遗漏刚入队的节点。
...
#### Condition 的底层实现（AQS ConditionObject）

> 🟠 **Condition 内部也是一个队列**——但和 AQS 的 CLH 队列不同：

```mermaid
flowchart TB
    subgraph AQS_CLH["AQS 同步队列 (争锁队列)"]
        H["head"] --> N1["Node(T1)<br/>ws=SIGNAL"]
        N1 --> N2["Node(T2)<br/>ws=0"]
    end

    subgraph COND_Q["Condition 等待队列 (条件队列)"]
        CF["firstWaiter"] --> C1["Node(T3)<br/>ws=CONDITION"]
        C1 --> C2["Node(T4)<br/>ws=CONDITION"]
    end

    C1 -.->|"signal() 时<br/>转移到 AQS 队列"| AQS_CLH
```
...
public final void await() throws InterruptedException {
    ...
    while (!isOnSyncQueue(node)) {         // ③ 循环：是否已被转移到 AQS 队列
        LockSupport.park(this);            //    没有 → 阻塞
    }
    // ④ 被 signal 转移到 AQS 队列后，重新竞争锁
    if (acquireQueued(node, savedState))   // 重新获取锁（恢复原来的 state）
        selfInterrupt();
}
...
public final void signal() {
    ...
    if (first != null)
        transferForSignal(first);  // 将 Condition 队列头节点转移到 AQS 队列尾部
    // 转移后该节点就像一个普通的锁等待者，按 AQS 规则竞争锁
}
```

> 🔴 **关键理解**：`await()` = 释放锁 + 入 Condition 队列 + park；`signal()` = 把节点从 Condition 队列移到 AQS 队列（但不释放锁！要等当前线程 unlock 后，AQS 队列中的节点才能获取锁）。这就是为什么 `signal()` 后必须尽快 `unlock()`——否则被唤醒的线程还是在 AQS 队列里等锁。
...
#### 线性探测的完整过程（get/set/remove）

> 🟠 **理解线性探测才能理解 ThreadLocal 的性能特征和 GC 清理机制**：
...
private void set(ThreadLocal<?> key, Object value) {
    Entry[] tab = table;
    int len = tab.length;
    int i = key.threadLocalHashCode & (len - 1);  // 计算起始槽位

    // ★ 线性探测：从 i 开始往后找
    for (Entry e = tab[i]; e != null; e = tab[i = nextIndex(i, len)]) {
        ThreadLocal<?> k = e.get();  // 弱引用取 key

        if (k == key) {
            e.value = value;  // 找到相同 key → 覆盖
            return;
        }
        if (k == null) {
            // ★★ key 被 GC 了（stale entry）→ 替换这个槽位 + 顺便清理
    ...
    int sz = ++size;
    // ★ cleanSomeSlots：启发式清理一部分过期 Entry
    if (!cleanSomeSlots(i, sz) && sz >= threshold)
        rehash();  // 清理后仍超阈值 → 扩容（2倍）
}
```
...
private Entry getEntry(ThreadLocal<?> key) {
    ...
    if (e != null && e.get() == key)
        ...
        // 未命中：线性探测向后找，同时清理过期 Entry
        return getEntryAfterMiss(key, i, e);
}
```

> 🟠 **性能关键**：由于黄金分割数的优秀散列特性，ThreadLocalMap 在正常使用下冲突极少（统计上几乎每次都是直接命中），get/set 均为 O(1)。只有在 ThreadLocal 数量极多（>16 且负载因子超 2/3）时才可能出现探测链变长的情况。
...
flowchart LR
    subgraph 清理前
        S0["[0] key=TL1 val=A"]
        S1["[1] key=null val=B<br/>★ stale"]
        S2["[2] key=TL3 val=C"]
        S3["[3] key=null val=D<br/>★ stale"]
        S4["[4] null"]
    end
    ...
    subgraph 清理后
        T0["[0] key=TL1 val=A"]
        T1["[1] null ✅ 已清理"]
        T2["[2] key=TL3 val=C<br/>(可能 rehash 到新位置)"]
        T3["[3] null ✅ 已清理"]
        T4["[4] null"]
    end

    清理前 -->|"expungeStaleEntry(1)"| 清理后
...
> 🟢 **为什么说"自清理不可靠"？** `expungeStaleEntry` 只在 get/set/remove **碰到**过期 Entry 时触发，它只清理从当前位置到下一个 null 槽位之间的过期条目。如果一个 ThreadLocal set 了之后再也没有被 get/set，它对应的 stale entry 可能永远不会被路过 → 永远不会被清理。这就是为什么 ==必须手动 remove()==。
...
> 🟠 **ConcurrentHashMap 的扩容不是单线程独占完成的**——它设计了多线程协助迁移机制，避免扩容时性能断崖：

```mermaid
flowchart TD
    A["某线程触发扩容<br/>(元素数超过阈值)"] --> B["创建新数组(2倍大小)<br/>设置 transferIndex = oldTable.length"]
    B --> C["将旧数组按 stride 分段<br/>(每段最少16个桶)"]
    C --> D["当前线程领取一段<br/>CAS: transferIndex -= stride"]
    D --> E["迁移该段中的每个桶"]
    E --> F["已迁移的桶放入 ForwardingNode<br/>(hash=MOVED=-1)"]

    G["其他线程执行 put/get"] --> H{"遇到 ForwardingNode?"}
    H -->|"put"| I["helpTransfer(): 也领取一段帮助迁移"]
    H -->|"get"| J["转发到新数组查找(不阻塞!)"]
    I --> D
...
> **文字版总结**：
> 1. 扩容时创建新数组，旧数组按段分配给多个线程并行迁移
> 2. 已完成迁移的桶位放置 `ForwardingNode`（标记节点），它的 hash 值是特殊的 MOVED（-1）
> 3. 后续线程 put 时如果遇到 ForwardingNode → 加入帮助迁移（不浪费 CPU）
> 4. 后续线程 get 时如果遇到 ForwardingNode → 通过它直接查新数组（读操作不阻塞！）
> 5. 这保证了扩容期间读写都不会完全阻塞——这是比 HashMap rehash（必须单线程、期间不能操作）巨大的优势
...
#### 为什么 LinkedBlockingQueue 用两把锁而 ArrayBlockingQueue 只用一把？

> 🟠 **这个设计差异的根本原因在于数据结构**：
>
> **ArrayBlockingQueue（一把锁）**：
> - 底层是 `Object[] items`（固定大小环形数组）
> - put 和 take 都需要修改 `putIndex`/`takeIndex`/`count` 这三个共享变量
> - 特别是 `count`——put 时 count++，take 时 count--，两者存在数据竞争
> - 所以必须用一把锁保护所有操作
>
> **LinkedBlockingQueue（两把锁）**：
> - 底层是链表：head 指针和 tail 指针指向不同的节点
> - put 只操作 tail 端（tail.next = newNode; tail = newNode）
> - take 只操作 head 端（head = head.next）
> - ==头尾操作物理上不同的内存位置，天然可并行==
> - count 用 `AtomicInteger` 原子操作（不需要锁保护）
>
> ```java
> // LinkedBlockingQueue 的两把锁设计
> private final ReentrantLock takeLock = new ReentrantLock();  // take 端独立锁
> private final Condition notEmpty = takeLock.newCondition();
> private final ReentrantLock putLock = new ReentrantLock();   // put 端独立锁
...
> ├── 是（防 OOM）
> │   └── 队列是否可能被高并发读写？
> │       ├── 是 → LinkedBlockingQueue(capacity) — 两把锁并行度高
> │       └── 否 → ArrayBlockingQueue(capacity) — 内存友好、GC 少
> └── 否
>     └── 什么场景？
>         ├── 零缓冲直接传递 → SynchronousQueue
>         ├── 延时任务 → DelayQueue
...
#### Worker 线程的 runWorker 循环（源码级剖析）

> 🔴 **Worker 线程创建后在做什么？** 它不停地从队列取任务并执行，直到线程池关闭或需要回收：

```java
// ThreadPoolExecutor.runWorker() — 每个工作线程的主循环
final void runWorker(Worker w) {
    Thread wt = Thread.currentThread();
    Runnable task = w.firstTask;  // ★ 第一个任务（创建时传入的）
    w.firstTask = null;
    w.unlock();  // 允许中断
    boolean completedAbruptly = true;  // ★ 标记：线程是否因异常退出

    try {
        // ★★ 核心循环：取任务 → 执行 → 再取 → 再执行...
        while (task != null || (task = getTask()) != null) {
            w.lock();  // Worker 自身的锁（用于 shutdown 时的中断判断）
            // 如果线程池正在 STOP，确保线程被中断
            ...
            try {
                beforeExecute(wt, task);  // 钩子方法（可重写，如设置 MDC）
                try {
                    task.run();           // ★★★ 真正执行用户的任务
                } catch (Throwable x) {
                    thrown = x; throw x;  // 异常透传（execute 场景线程会死）
                } finally {
                    afterExecute(task, thrown);  // 钩子方法（可重写，如清理/报警）
                }
            } finally {
                task = null;
                w.completedTasks++;
...
private Runnable getTask() {
    ...
    for (;;) {
        ...
        // 线程池已 SHUTDOWN 且队列空 → 返回 null → 线程退出
        ...
        // ★ 决定是用 poll(timeout) 还是 take()
        // allowCoreThreadTimeOut=true 或 当前线程数>核心数 → 用 poll（可超时回收）
        ...
        try {
            // ★ 核心线程用 take() 永远阻塞等待（不会超时回收）
            // ★ 非核心线程用 poll(keepAliveTime) 限时等待
            Runnable r = timed ?
                workQueue.poll(keepAliveTime, TimeUnit.NANOSECONDS) :
                workQueue.take();
            if (r != null) return r;
            timedOut = true;  // poll 超时了 → 标记，下次循环可能回收
...
> 🔴 **核心理解**：
> - **核心线程为什么不会被回收？** 因为它用 `workQueue.take()`，永远阻塞直到有新任务——不会超时返回 null。
> - **非核心线程为什么会被回收？** 因为它用 `workQueue.poll(keepAliveTime)`，超时后返回 null → getTask 返回 null → runWorker 循环结束 → processWorkerExit 清理。
> - **completedAbruptly 标记的作用？** 如果是 `true`（异常退出），processWorkerExit 会创建一个新线程替补；如果是 `false`（正常回收），只是简单退出。
...
> 🟠 **ForkJoinPool 的 WorkQueue 是一个无锁的双端队列**（基于数组 + CAS），理解它才能理解为什么工作窃取几乎没有开销：

```mermaid
flowchart TB
    subgraph WorkQueue["WorkQueue (数组实现的 Deque)"]
        direction LR
        S["base<br/>(偷取端)"] --> ARR["[task5][task4][task3][task2][task1]"]
        ARR --> T["top<br/>(自取端)"]
    end

    OWN["Owner 线程<br/>pop(): CAS top--<br/>取 task1"] --> T
    THIEF["偷取线程<br/>poll(): CAS base++<br/>取 task5"] --> S

    style OWN fill:#48dbfb
    style THIEF fill:#feca57
```

> **无锁关键**：
> - **Owner pop（自取）**：绝大多数时候只有 Owner 操作 top 端，直接 `top--` + 简单 CAS（只有队列仅剩 1 个元素时才需要和偷取者竞争）
> - **Thief poll（偷取）**：CAS `base++`，如果失败说明有其他偷取者竞争，重试或换一个队列偷
> - **竞争窗口极小**：Owner 操作头部，Thief 操作尾部，只有队列几乎空时才可能冲突
>
> **这比传统锁好在哪里？** 假设有 8 个工作线程，传统锁方案下所有线程共享一个任务队列（一把锁 → 串行化）。ForkJoinPool 每个线程有自己的队列，正常情况零竞争；只有忙完了才去偷——而且偷的是大任务（递归分治的顶层），偷一次能忙很久。所以偷取频率很低，竞争几乎不存在。
...
> 🔴 **RingBuffer 不是普通的环形队列**——它预分配对象+序列号发布，理解这两点才能理解为什么零 GC：

```mermaid
flowchart LR
    subgraph RingBuffer["RingBuffer (大小=8, 2^3)"]
        direction LR
        E0["[0] Event<br/>(预分配)"]
        E1["[1] Event"]
        E2["[2] Event"]
        E3["[3] Event<br/>★cursor=3"]
        E4["[4] Event"]
        E5["[5] Event"]
        E6["[6] Event"]
        E7["[7] Event"]
    ...
    P["生产者<br/>sequence=4"] -->|"1. CAS claim seq=4<br/>2. 定位: 4&7=4<br/>3. 写入 Event[4]<br/>4. publish(4)"| E4
    C["消费者<br/>已处理到 seq=2"] -->|"等待 cursor≥3<br/>批量处理 [3]"| E3

    style E3 fill:#feca57
    style E4 fill:#48dbfb
```
...
```java
// Disruptor 生产者写入步骤
long sequence = ringBuffer.next();         // ① CAS 抢占下一个序列号（原子操作）
try {
    Event event = ringBuffer.get(sequence); // ② 获取预分配的 Event 对象（不是 new！）
    event.setValue(data);                   // ③ 填充数据（覆盖旧值）
} finally {
    ringBuffer.publish(sequence);           // ④ 发布：更新 cursor，通知消费者
}
// ★ 整个过程没有 new 任何对象 → 零 GC
// ★ 定位用 sequence & (bufferSize-1) → 等价于 % 但更快
```

> 🔴 **为什么预分配消除了 GC？** 
> - 传统队列：每个消息都 `new Message(data)` → 消费后 Message 变成垃圾 → GC 回收
> - Disruptor：启动时 `new Event[size]` 分配好所有对象，永远不释放。生产者只是覆盖旧 Event 的字段值。对象本身永远活在 Old Gen，不会触发 GC。
>
> **为什么位运算代替取模？**
> - `sequence % size` 需要除法指令（CPU 约 20~40 个时钟周期）
> - `sequence & (size-1)` 只需 AND 指令（1 个时钟周期）
> - 前提：size 必须是 2^N（强制约束）
...
>
> **为什么"不精确"通常不是问题？** 在实际使用中，你拿到 size() 的返回值那一刻，其他线程可能已经又 put 了新元素——即使 size() 是精确的瞬时快照，你使用它的时候也已经"过时"了。所以在并发环境下，精确的 size 本身就没有实际意义，它只能作为一个"大致估计"。
>
> **面试加分**：JDK 7 的分段锁版本 size() 更有意思——先无锁尝试两次（每次统计所有 Segment 的 modCount，两次一样说明期间没修改），如果不一致就给所有 Segment 加锁再统计。这是一个"乐观→悲观"的渐进策略，和 CAS 失败后升级为锁是同一个思路。
...
>
> **场景4：迭代器与修改的语义差异**。COW 的 iterator 永远不会抛 ConcurrentModificationException（因为迭代的是快照），但这也意味着迭代器不反映最新修改——如果业务依赖"遍历时能看到最新数据"，COW 做不到。
>
> **适用场景总结**：配置列表、监听器/观察者列表、白名单/黑名单——写入极少（启动时/配置变更时），读取极频繁（每次请求都读）。写入频率经验阈值：**写入 < 读取的 1/1000** 才适合 COW。
>
> **面试加分**：Spring 的 `ApplicationEventMulticaster` 存储监听器列表就用了类似 COW 的思路——启动时注册（低频写），事件发布时遍历（高频读）。Tomcat 的 Mapper（URL→Servlet 映射）也是类似策略。
...
>
> 4. **级联阻塞**：如果调用者本身也有超时限制（如 HTTP 请求 30s 超时），CallerRuns 执行的任务可能导致原始请求超时失败，影响面从"一个异步任务失败"扩大为"整个请求失败"。
>
> **什么场景适用**：
> - ✅ 任务不能丢且系统可以接受变慢（如日志写入、非实时数据同步、批处理作业）
> - ✅ 调用者不是请求线程（如定时任务调用者线程）
> - ❌ 不适用于 HTTP 请求处理链路中的线程池
>
> **更好的生产方案**：
> ```java
> // 自定义拒绝策略：报警 + 持久化 + 降级
...
>     @Override
>     public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
>         // 1. 报警（钉钉/企微/PagerDuty）
>         alertService.sendUrgent("线程池 " + poolName + " 过载!");
>         // 2. 持久化到 MQ（后续补偿执行）
...
> ```
>
> **面试加分**：Spring 的 `@Async` 默认使用 SimpleAsyncTaskExecutor（每次 new Thread！），如果要用 CallerRunsPolicy，必须自己配置 ThreadPoolTaskExecutor。但请注意——Spring MVC 中 @Async + CallerRunsPolicy 意味着当线程池满时，请求线程自己执行异步任务，相当于异步退化为同步。
